...
class APIRadars(RequestHandler):
  def get(self):
    user = self.GetCurrentUser()
    if (not user):
...
class APIComments(RequestHandler):
  def get(self):
    user = self.GetCurrentUser()
    if (not user):
      self.respondWithDictionaryAsJSON({"error":"please authenticate by setting the Authorization header to your API key (http://www.openradar.me/apikey)"})
...
class APIRadarsNumbers(RequestHandler):
  def get(self):
    page = self.request.get("page")
    if page:
      page = int(page)
...
class APIRadarsIDs(RequestHandler):
  def get(self):
    page = self.request.get("page")
    if page:
      page = int(page)
...
class APIAddRadar(RequestHandler):
  def post(self):
    user = self.GetCurrentUser()
    if (not user):
...
class APISecret(RequestHandler):
  def get(self):
...
class APIRecentRadars(RequestHandler):
  def get(self):
    user = self.GetCurrentUser()
    if (not user):
      self.respondWithDictionaryAsJSON({"error":"please authenticate by setting the Authorization header to your API key (http://www.openradar.me/apikey)"})
...
class APIRecentComments(RequestHandler):
  def get(self):
    user = self.GetCurrentUser()
    if (not user):
      self.respondWithDictionaryAsJSON({"error":"please authenticate by setting the Authorization header to your API key (http://www.openradar.me/apikey)"})
